  ...
  return (
    <div className="space-y-6 sm:space-y-8 p-3.5 sm:p-6 md:p-8 pt-4 sm:pt-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight font-heading">Dashboard</h2>
        ...
        <Link href="/invoices/new" className="w-full sm:w-auto">
          <Button className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl shadow-xs cursor-pointer font-semibold px-5 py-2.5">
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        <Card className="rounded-2xl border bg-card/60 backdrop-blur shadow-xs hover:shadow-sm transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-3.5 sm:p-5 pb-1 sm:pb-2">
        ...
        <Card className="rounded-2xl border bg-card/60 backdrop-blur shadow-xs hover:shadow-sm transition-all">
          ...
          <CardContent className="p-3.5 sm:p-5 pt-0">
            <div className="text-lg sm:text-2xl font-bold text-emerald-600 dark:text-emerald-400 font-heading">
              ${metrics.paid.toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </div>
            <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 truncate">Settled payments</p>
          </CardContent>
        </Card>

        <Card className="rounded-2xl border bg-card/60 backdrop-blur shadow-xs hover:shadow-sm transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-3.5 sm:p-5 pb-1 sm:pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground truncate">Drafts</CardTitle>
            <div className="p-1.5 sm:p-2 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 shrink-0">
              <FileText className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
            </div>
          </CardHeader>
          <CardContent className="p-3.5 sm:p-5 pt-0">
            <div className="text-lg sm:text-2xl font-bold font-heading">{metrics.draftCount}</div>
            <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 truncate">Unpublished</p>
          </CardContent>
        </Card>

        <Card className="rounded-2xl border bg-card/60 backdrop-blur shadow-xs hover:shadow-sm transition-all">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-3.5 sm:p-5 pb-1 sm:pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground truncate">Total Invoiced</CardTitle>
          ...
          <CardContent className="p-3.5 sm:p-5 pt-0">
            ...
            <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 truncate">{metrics.totalCount} invoices total</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
        <div className="flex overflow-x-auto scrollbar-none pb-1 gap-1.5 sm:gap-2 touch-scroll -mx-4 px-4 sm:mx-0 sm:px-0">
              ...
              {filteredInvoices.map((inv) => (
                <div key={inv.id} className="p-4 space-y-3 hover:bg-muted/20 transition-colors">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl">
                          ...
                          <DropdownMenuItem onClick={() => handleCopyLink(inv.id)} className="cursor-pointer flex items-center">
                            <Copy className="mr-2 h-3.5 w-3.5 text-muted-foreground" />
                            Copy Share Link
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleMoveToTrash(inv)} className="text-destructive cursor-pointer flex items-center">
                  ...
                  <div className="flex items-baseline justify-between">
                    <div>
                      <p className="font-medium text-foreground text-sm">{inv.client}</p>
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-border/40">
                    <Link href={`/invoices/new?id=${encodeURIComponent(inv.id)}`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full h-8 text-xs rounded-xl">
                        Edit
                      </Button>
                    </Link>
                    <Link href={`/share/${encodeURIComponent(inv.id)}`} target="_blank" className="flex-1">
                      <Button variant="outline" size="sm" className="w-full h-8 text-xs rounded-xl">
                        <ExternalLink className="h-3.5 w-3.5 mr-1 text-muted-foreground" /> View
                      </Button>
                    </Link>
                    <a
                      ...
                      className="inline-flex items-center justify-center h-8 px-2.5 rounded-xl bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors text-xs font-medium"
