      ...
      <table className="w-full text-sm text-left min-w-[480px]">
        <thead>
          <tr className={`border-gray-200 ${modern ? 'border-b-2 border-black' : 'border-y-2'}`}>
            ...
            <th className={`py-3 text-center px-4 ${modern ? 'font-bold uppercase text-xs tracking-wider' : 'font-semibold'}`}>Qty</th>
          ...
          {data.items.map(item => (
            <tr key={item.id}>
              <td className="py-4">
                ...
                {item.hsnCode && <div className="text-[11px] text-gray-500 font-mono">SKU: {item.hsnCode}</div>}
              </td>
              <td className="py-4 text-center px-4 text-gray-600">{item.quantity} {item.unit}</td>
...
export function BankDetails({ data }: { data: InvoiceData }) {
  ...
  return (
    <div className="text-sm text-gray-600 z-10 mt-6 bg-gray-50 p-4 rounded-md relative">
      <p className="font-bold text-gray-900 mb-2 uppercase text-xs tracking-wider">Wire & Bank Details</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        {data.accountNumber && <div><span className="font-semibold">Account / IBAN:</span> {data.accountNumber}</div>}
        {data.ifscCode && <div><span className="font-semibold">Routing:</span> {data.ifscCode}</div>}
        {data.swiftCode && <div><span className="font-semibold">SWIFT / BIC:</span> {data.swiftCode}</div>}
